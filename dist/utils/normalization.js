...
exports.normalizeIngredients = normalizeIngredients;
...
function normalizeIngredientName(ingredient) {
    ...
    normalized = normalized
        .replace(/\b(fresh|dried|frozen|cooked|raw|roasted|grilled|ground|ground|whole|halved|sliced|chopped|minced|diced|crushed|natural|organic|pre|light|dark)\b/gi, '')
        .replace(/\s+/g, ' ')
        .trim();
...
function parseIngredientString(raw) {
    ...
    const match = trimmed.match(/^(\d+(?:\.\d+)?)\s+(cups?|tbsps?|tsps?|ml|l|g|kg|oz|lbs?|cans?|jars?|pinch|dash)\s+(.+)$/i);
    if (match) {
